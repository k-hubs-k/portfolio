...
const Terminal = () => {
  ...
  return (
    <>
          ...
          {history.map((entry, index) => {
            ...
            return (
              <div
                ...
                className={`
                  mb-3 sm:mb-4
                  ${isLastEntry ? 'bg-[rgba(78,205,196,0.05)] border-l-2 border-[rgba(78,205,196,0.3)] pl-3 py-2 rounded-r' : ''}
                `}
              >
                {entry.command && (
      ...
      {/* FloatingTourButton moved OUTSIDE the overflow-hidden container */}
      {/* Button is always visible to allow users to replay the tour */}
      <FloatingTourButton
        ...
        onSkip={skipTour}
        currentStep={isTourActive ? getCurrentStep() : null}
        onStart={startTour}
      />
    </>
  );
